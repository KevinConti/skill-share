...
pub fn cli_version_test() {
  let assert Ok(output) = skill_universe.run(["version"])
...
pub fn cli_unknown_command_test() {
  let result = skill_universe.run(["unknown"])
...
pub fn cli_compile_all_test() {
  let output_dir = "/tmp/skill_universe-cli-test-compile-all"
...
pub fn cli_compile_single_target_test() {
  let output_dir = "/tmp/skill_universe-cli-test-compile-target"
  let _ = simplifile.delete(output_dir)
...
pub fn cli_compile_providers_flag_test() {
  let output_dir = "/tmp/skill_universe-cli-test-providers"
  let _ = simplifile.delete(output_dir)
...
pub fn cli_compile_nonexistent_dir_test() {
  let result = skill_universe.run(["compile", "/tmp/nonexistent-dir-xyz"])
  should.be_error(result)
}
...
pub fn cli_check_nonexistent_dir_test() {
  let result = skill_universe.run(["check", "/tmp/nonexistent-dir-xyz"])
  should.be_error(result)
  let assert Error(msg) = result
...
pub fn cli_check_invalid_yaml_test() {
  let result = skill_universe.run(["check", "test/fixtures/invalid-yaml"])
  should.be_error(result)
}
...
pub fn cli_init_test() {
  let dir = "/tmp/skill_universe-cli-test-init"
  let _ = simplifile.delete(dir)

  let assert Ok(output) = skill_universe.run(["init", dir])
  should.be_true(string.contains(output, "Created skill"))

  // Name should be derived from path
  should.be_true(string.contains(output, "skill_universe-cli-test-init"))
...
pub fn cli_init_already_exists_test() {
  let dir = "/tmp/skill_universe-cli-test-init-exists"
  let _ = simplifile.delete(dir)

  let assert Ok(_) = skill_universe.run(["init", dir])
  let result = skill_universe.run(["init", dir])
  should.be_error(result)
  let assert Error(msg) = result
...
pub fn name_from_path_simple_test() {
  should.equal(skill_universe.name_from_path("my-skill"), "my-skill")
}

pub fn name_from_path_nested_test() {
  should.equal(skill_universe.name_from_path("/home/user/my-skill"), "my-skill")
}
